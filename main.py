...
def process_image(img):
    ...
    for i in range(len(hTracklist)):
        ...
        contours, hierarchy = cv2.findContours(tempImg, 1, 2)
            ...
            if not allMode:
                ...
                if len(record) > 1 and showing:
                    for i in range(len(record) - 1):
                        img = cv2.line(img, (record[i][0], record[i][1]), (record[i + 1][0], record[i + 1][1]),
                                       (150, 0, 150))
            else:
                if recording:
                ...
                if len(recordList) > 1 and len(recordList[i]) > 1 and showing:
                    ...
                    for j in range(len(recordList[i]) - 1):
                        img = cv2.line(img, (recordList[i][j][0], recordList[i][j][1]),
                                       (recordList[i][j + 1][0], recordList[i][j + 1][1]),
                                       (colorList[i][0], colorList[i][1], colorList[i][2]))
    return img
...
def displayText():
    ...
    if hueRecal:
        cv2.putText(img, "Press b to disable real-time hsv recalibration for all objects", (start, start + 5 * offset),
                    font, scale, color)
    else:
        cv2.putText(img, "Press b to enable real-time hsv recalibration for all objects", (start, start + 5 * offset),
                    font, scale, color)
    cv2.putText(img, "Press h to recalibrate hsv for the selected object now", (start, start + 6 * offset), font, scale,
                color)
    ...
    if allMode:
        cv2.putText(img, "Your action will effect ALL tracked objects", (start, start + 11 * offset), font, scale,
                    color)
    else:
        cv2.putText(img, "Your action will effect ONLY THE SELECTED object", (start, start + 11 * offset), font, scale,
                    color)
